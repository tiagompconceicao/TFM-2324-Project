  ...
  return {
    ...
    send: async ()  => {
      ...
      return response.ok && isJson(response.headers.get('Content-Type')) ?
        { header: response, body: await response.text().then(text => text ? JSON.parse(text) : undefined) } :
        { header: response }
    }
  }
}
...
export type FirebaseConfig = {
  ...
  export function loadFirebaseConfig(url: URL): Request<FirebaseConfig> {
    return cancelableRequest(url, {
      headers: { },
    })
  }
  
  export function fetchInfo(url: URL): Request<FirebaseConfig> {
    return cancelableRequest(url, {
      headers: { },
    })
  }